...
export default function Programmers() {
  ...
  const [programmersId, setProgrammer] = useState(null);
      ...
      <div className='programmers'>
        {
         ...
         programmersList.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => { setProgrammer(currentDev => currentDev = dev.id)/* in here set the featured id to be dev.id */ }}>Feature</button>
            </div>
          )
        }
      </div>
